...
class FailuresExtractor {
    ...
    _findReturnedFailures(method) {
        ...
        const failures = newStatementsInReturn
            .filter((exp) => {
            return typescript_1.default.isIdentifier(exp.expression) && exp.expression.escapedText == response_1.FailureResponse.name;
        })
            ...
            if (!exp.arguments || exp.arguments.length < 2)
                return undefined;
            const codeArg = exp.arguments.at(0);
            const failureArg = exp.arguments.at(1);
            let code = 400;
            let failure = '';
            if (codeArg && typescript_1.default.isNumericLiteral(codeArg)) {
                code = Number.parseInt(codeArg.text);
            }
            ...
            return new response_1.FailureResponse(code, failure);
        })
